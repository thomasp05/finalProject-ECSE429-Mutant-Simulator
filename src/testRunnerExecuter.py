...
def main():
    ...
    for i in range(len(fileNameList)):
        try:
            ...
            pool.apply_async(testRunnnerMethod, args=(sutPath,path + fileNameList[i], i+1),callback=get_result )
            #uncomment the following code if you dont want to execute the loop in parallel
            # results.append(testRunnnerMethod(sutPath,path + fileNameList[i], i+1))
